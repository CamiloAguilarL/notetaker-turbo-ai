...
| UI-01 | P1 | Use one application scrollbar treatment. | Owner | Every vertical and horizontal scroll surface uses the same thin rounded track/thumb contract; the document and nested controls support Firefox and WebKit, hover remains visible, forced-colors can use the system treatment, and the editor may inherit its category accent without changing geometry. |
